...
@Component({
  selector: 'nc-virtualized-table-header',
  templateUrl: './virtualized-table-header.html',
  styleUrl: './virtualized-table-header.css',
  imports: [CommonModule],
  standalone: true
})
export class VirtualizedTableHeader implements OnInit, OnDestroy {
  ...
  getThClass(col: Column): string {
    const classes = ['virtualized-table-header-th'];
    if (this.draggedColumn === col.column) classes.push('virtualized-table-header-dragged');
    if (this.dragOverColumn === col.column) classes.push('virtualized-table-header-drag-over');
    if (this.isVerticalSeparator(col)) classes.push('virtualized-table-header-vertical-separator');
    return classes.join(' ');
  }
  ...
  private updateGhostElement(): void {
    ...
    if (this.ghostElement) {
      this.ghostDiv.innerHTML = `
        <div class="virtualized-table-header-ghost" style="
          position: fixed;
